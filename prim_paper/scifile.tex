...
Hovhannes Bantikyan from the University of Armenia did time measurements up to the size of 200000 digits for each factor and compared the results with other bignum multiplication libraries \cite{bantikyan2014big}. In contrast to that, the results of cuFFT are compared with the \mintinline{c}{BN_mul()} function in this paper. The factors are also measured up in smaller steps of size 24 bits. Ando Emerencia also did time measurements in his paper "Multiplying huge Integers using Fourier Transforms" \cite{emerencia2007multiplying}. He did time measurement with different bases and Maximum Square Error tests.
...
\begin{center}
$f_m = \sum_{k=0}^{2n-1}x_k*e^{-\frac{2\pi*i}{2n}mk} \; m = 0,\dots,2n-1$.
\end{center}
...
GPUs that are used in modern computers do have hundreds of cores and many more threads, thus they are very effective in parallel computing \cite{Palacios2011ACO}. Parallel Computing speeds up the process when multiplying two numbers, because multiplication always is a process of many tiny multiplications that are summed up. Computations on the CPU are preferred when multiplying tiny numbers, because of the low memory access times \cite{albers2009grafik}.
...
\end{figure}
 ...
 With Numbers by Apple, the second grade polynomial of GPU\_Calc and CPU were computed and visualized in figure \ref{cpugpu_times} with the original graphs.
...
\end{center}
 ...
 It can be derived, that the slope of the blue polynomial is decreasing, while the slope of the green polynomial is increasing.
